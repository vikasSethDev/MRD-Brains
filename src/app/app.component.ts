...
export class AppComponent implements OnInit {
  showTop = false; waOpen = false; showBadge = true; showTooltip = false;
  ...
  ngOnInit() {
    if (isPlatformBrowser(this.pid)) {
      if (typeof AOS !== 'undefined') AOS.init({ duration: 780, once: true, offset: 50, easing: 'ease-out-cubic' });
  ...
  toggleWA() { this.waOpen = !this.waOpen; if (this.waOpen) { this.showBadge = false; this.showTooltip = false; } }
  sendWA(msg: string) { window.open(`https://wa.me/${this.waNumber}?text=${encodeURIComponent(msg)}`, '_blank'); }
}